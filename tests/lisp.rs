...
fn test_parser() {
    ...
    assert_eq!(parse("(- herbie0)"), Ok(Unary(UnNeg, box Ident(0))));
    assert_eq!(parse("(- herbie0 herbie1)"), Ok(Binary(BiSub, box Ident(0), box Ident(1))));

    assert_eq!(parse("-"), Err(ParseError::Unexpected('-')));
    assert_eq!(parse("(-)"), Err(ParseError::Unexpected(')')));
    assert_eq!(parse("(- 0. 0. 0.)"), Err(ParseError::Expected(')')));

    assert_eq!(parse("(cos 1.)"), Ok(Fun("cos".into(), vec![Lit(1.)])));
    assert_eq!(parse("(cos herbie0)"), Ok(Fun("cos".into(), vec![Ident(0)])));
    assert_eq!(parse("(log1p (cos herbie0))"), Ok(Fun("log1p".into(), vec![Fun("cos".into(), vec![Ident(0)])])));
